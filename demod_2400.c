...
void demodulate2400(struct mag_buf *mag) {
    ...
    mag_t sum_scaled_signal_power = 0.0;
    ...
    for (j = 0; j < mlen; j++) {
        ...
        if (base_signal < g_dbusvars.demod_snr1 * base_noise) // about 3dB SNR
            continue;
...
void demodulate2400AC(struct mag_buf *mag) {
    ...
    mag_t noise_stddev = sqrt(mag->mean_power - mag->mean_level * mag->mean_level); // Var(X) = E[(X-E[X])^2] = E[X^2] - (E[X])^2
    ...
    for (f1_sample = 1; f1_sample < mlen; ++f1_sample) {
        ...
        if (noise_level * g_dbusvars.demod_snr2 > f1_level) {
        ...
        if (noise_level * g_dbusvars.demod_snr2 > f2_level) {
            // require 5dB above noise
            continue;
        }
